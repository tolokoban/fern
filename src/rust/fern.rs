...
pub fn barnsley_fern_paint(
    ...
    unsafe {
        while loops > 0 {
            ...
            if random < func1.p {
                func1.compute(&mut point);
            } else if random < func2.p {
                func2.compute(&mut point);
            } else if random < func3.p {
                func3.compute(&mut point);
            } else {
                func4.compute(&mut point);
            }
...
#[wasm_bindgen]
pub fn barnsley_fern_compute_bounds(
    data: &mut [f32],
    seed: u32,
    f1a: f32,
    f1b: f32,
    ...
    unsafe {
        while loops > 0 {
            ...
            let min_x = data.get_unchecked_mut(0);
            *min_x = min(*min_x, point.x);
            let min_y = data.get_unchecked_mut(1);
            *min_y = min(*min_y, point.y);
            let max_x = data.get_unchecked_mut(2);
            *max_x = max(*max_x, point.x);
            let max_y = data.get_unchecked_mut(3);
